...
const emojiCache = new LruMap(1024);
export async function loadEmoji(type_, code) {
    let type = type_;
    const key = `${type}:${code}`;
    if (emojiCache.get(key))
    ...
    const response = await fetch(`${api}${code.toUpperCase()}.svg`).then((r) => r.text());
    emojiCache.set(key, response);
    return response;
}
